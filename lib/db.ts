...
/**
 * Resolve the DB user id (uuid) for an email — the identity backing the
 * `cf_uid` session cookie. Returns null when Supabase is not configured or the
 * user has no row. Used by mutations that must attribute rows to the
 * authenticated user rather than a client-supplied id.
 */
export async function getUserIdByEmail(email: string): Promise<string | null> {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) {
    console.error("[DB] getUserIdByEmail error:", error.message);
    return null;
  }
  return data?.id ?? null;
}
